...
/** Event reason mapping to English labels */
...
/** Get English label for event reason
...
export interface TimeSlot {
  ...
  eventReason?: string; // Event reason label (e.g., 'User Interaction Dashcam Icon Tapped', 'Sentry Aware Object Detection')
      ...
      if (sources.includes('unknown') && slotData.reason) {
        ...
        if (nonUnknownSources.length === 0) {
          ...
          if (reasonLower.includes('sentry')) {
            sources = ['sentry'];
          } else if (reasonLower.includes('user interaction') || reasonLower.includes('honk') || reasonLower.includes('emergency') || reasonLower.includes('braking') || reasonLower.includes('collision')) {
            sources = ['saved'];
          }
        }
      }
